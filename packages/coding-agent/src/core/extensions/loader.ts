...
async function loadExtensionModuleResult(
	extensionPath: string,
	cwd: string,
	...
	try {
		...
		return factory
			? { extensionPath, resolvedPath, factory }
			: {
					extensionPath,
					resolvedPath,
...
async function loadExtensionsInternal(
	...
	// Module evaluation is independent; factories still run in configured order
	// because their registrations mutate shared runtime state.
	const modules = await Promise.all(
		paths.map((extensionPath) => loadExtensionModuleResult(extensionPath, resolvedCwd, cacheToken)),
	);
